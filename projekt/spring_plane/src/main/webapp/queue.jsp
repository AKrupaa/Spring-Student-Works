...
<main class="container">
    ...
    <c:if test="${!empty confirmed}">
        <h2 class="alert alert-info">${confirmed}</h2>
    </c:if>
    <c:if test="${!empty cancelled}">
        <h2 class="alert alert-info">${cancelled}</h2>
    </c:if>
    <c:if test="${!empty notFound}">
        <h2 class="alert alert-info">${notFound}</h2>
    </c:if>
        ...
        <table class="table mt-3">
            ...
            <c:forEach items="${tickets}" var="ticket" varStatus="status">
                <tr>
                    ...
                    <td>${ticket.price}</td>
                    <td><a href="/queue/confirm/${ticket.id}">Confirm</a></td>
                    <td><a href="/queue/cancel/${ticket.id}">Cancel</a></td>
                        <%--                    <td>YES</td>--%>
                </tr>
            </c:forEach>
            </tbody>
    ...
    <c:if test="${empty tickets}">
        <h2 class="mt-5">Nothing to do :)</h2>
    </c:if>
</main>
